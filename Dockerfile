...
# Python + that build output: server.py serves both /api/* and the built
# dist/ from one process on one port, so nothing in the container needs to
# know about cross-origin requests at all -- that's purely a dev-mode concern
# (see vite.config.ts's /api proxy), not a packaged one.
